 ...
 * 该类不适用于local模式的spark中,这个时候executor  backend  master都运行在同一个JVM中
 * It sits behind a TaskSchedulerImpl and handles launching tasks on a single Executor (created by the LocalBackend) running locally.
 */
