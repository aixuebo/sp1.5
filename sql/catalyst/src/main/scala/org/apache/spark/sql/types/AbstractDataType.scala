...
private[sql] abstract class AbstractDataType {
  ...
  /** Readable string representation for the type. 该类型的可读的描述信息*/
 ...
 * 这意味着我们更喜欢StringType,喜欢程度已经超过了BinaryType
 */
private[sql] class TypeCollection(private val types: Seq[AbstractDataType])
  ...
  override private[sql] def defaultConcreteType: DataType = types.head.defaultConcreteType //第一个类型是默认类型

  //只要有任意一个AbstractDataType 接收DataType类型就返回true
...
private[sql] object TypeCollection {
  ...
  /**
   * Types that can be ordered/compared.
   * 类型能够排序或者比较
   * In the long run we should probably make this a trait
  ...
  //将一组类型组装成TypeCollection对象
  def apply(types: AbstractDataType*): TypeCollection = new TypeCollection(types)

  //从TypeCollection对象中抽取包含哪些类型
...
private[sql] object NumericType extends AbstractDataType {
  ...
  override private[sql] def defaultConcreteType: DataType = DoubleType //默认是double类型
