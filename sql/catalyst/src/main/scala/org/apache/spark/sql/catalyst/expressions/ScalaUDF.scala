 ...
 * 用户自定义函数
 * @param dataType  Return type of function. 数据返回类型是一个函数,而不是具体的值
 */
case class ScalaUDF(
    function: AnyRef,//一个自定义的函数,例如() => Any 表示没有参数,返回一个any,或者有最多20个参数,例如2个参数(Any, Any) => Any,该函数是每一行数据作为参数进行调用该函数
    dataType: DataType,//返回的数据类型
    children: Seq[Expression],//需要的表达式集合
    inputTypes: Seq[DataType] = Nil)//输入类型
  ...
  */
  ...
  private[this] val f = children.size match {
    case 0 =>
      val func = function.asInstanceOf[() => Any] //没有参数的函数
      (input: InternalRow) => { //每一行数据去调用该函数
        func()
      }

    case 1 =>
      val func = function.asInstanceOf[(Any) => Any] //一个参数的函数
      val child0 = children(0)
      lazy val converter0 = CatalystTypeConverters.createToScalaConverter(child0.dataType) //child0表达式的返回值转换器
      (input: InternalRow) => {
        func(
          converter0(child0.eval(input))) //child0.eval(input) 表示对该表达式进行运算,返回一个any对象,然后转换成child0需要的数据类型
      }
  ...
  // scalastyle:on
  private[this] val converter = CatalystTypeConverters.createToCatalystConverter(dataType) //返回值类型的转换函数
  override def eval(input: InternalRow): Any = converter(f(input)) //调用f函数,将一行数据传入进去,返回一个any对象,然后将该对象转换成输出类型
}
