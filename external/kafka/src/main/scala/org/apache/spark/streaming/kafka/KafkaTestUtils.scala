...
private[kafka] class KafkaTestUtils extends Logging {
  ...
  /** Java-friendly function for sending messages to the Kafka broker
    * messageToFreq 表示map中的key发送value次
    **/
  ...
  def sendMessages(topic: String, messageToFreq: Map[String, Int]): Unit = {
    val messages = messageToFreq.flatMap { case (s, freq) => Seq.fill(freq)(s) }.toArray //数组数据组成是发送String字符串Int次
    sendMessages(topic, messages) //向topic发送一个数组数据
  }

  /** Send the array of messages to the Kafka broker */
  def sendMessages(topic: String, messages: Array[String]): Unit = {
    producer = new Producer[String, String](new ProducerConfig(producerConfiguration)) //创建producer
    producer.send(messages.map { new KeyedMessage[String, String](topic, _ ) }: _*) //发送String,String类型数据
    producer.close() //发送完毕
    producer = null
  }
